# 科研选题与研究问题表

这个资料包用于从“模糊兴趣”走到“可检索、可验证、可写作”的研究问题。

## Files

| 文件 | 用途 |
| --- | --- |
| [templates/pico-planner.md](./templates/pico-planner.md) | PICO 研究问题拆解 |
| [templates/research-question-table.csv](./templates/research-question-table.csv) | 选题对比表 |
| [templates/literature-search-log.csv](./templates/literature-search-log.csv) | 文献检索记录表 |
| [templates/evidence-gap-map.md](./templates/evidence-gap-map.md) | 研究空白地图 |

## Recommended Workflow

1. 写下 3-5 个候选研究问题。
2. 用 PICO 表拆解每个问题。
3. 用 Suppr 或 PubMed 做快速检索，记录命中数量和关键文献。
4. 用研究空白地图判断是否值得继续。
5. 选择一个资料足够、问题清晰、可执行的方向。

Suppr 官网：https://suppr.wilddata.cn/
